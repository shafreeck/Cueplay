...
exports.QuarkProvider = void 0;
class QuarkProvider {
    static API_URL = 'https://drive-pc.quark.cn/1/clouddrive/file/v2/play?pr=ucpro&fr=pc';
    async resolvePlayableSource(fileId, context) {
        if (!context.cookie) {
        ...
        if (!playUrl && data.data?.video_list && Array.isArray(data.data.video_list)) {
            ...
            const stereoStream = list.find((v) => v.video_info?.audio?.channels === 2 && v.video_info?.url);
            if (stereoStream) {
                playUrl = stereoStream.video_info.url;
            }
            else {
                // Fallback to the first available URL
                const target = list.find((v) => v.video_info?.url);
                if (target) {
